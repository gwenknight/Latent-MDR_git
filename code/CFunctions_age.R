  ...
  for (k in year1:(yearend)){
    ...
    if (k > year1){ # Start of year 1 is the initial condition
      ...
      if (i==length(seq(year1,yearend,dt))){ # LAST TIME STEP
        ...
        #### CHANGED THIS TO INCLUDE CASES AND DEATHS BY AGE
        hbcOut<-matrix(0,length(yrcount)-1,22)
        hbcOut[,1]<-seq(year1,yearend-1)
        nns2<-c("Year","Psize","negcases","poscases","negdeaths","posdeaths",
                              "negcases_i","poscases_i","negdeaths_i","posdeaths_i",
                               "negcases_c","poscases_c","negdeaths_c","posdeaths_c",
                                "negcases_a_a","poscases_a_a","negdeaths_a_a","posdeaths_a_a",
                                "negcases_a","poscases_a","negdeaths_a","posdeaths_a")
        ...
        for (i in 1:(length(yrcount)-1)){
          ...
          ### TO GET AGE GROUP OUTPUTS I THINK WE NEED TO SUM OVER THE CORRECT COLUMNS OF new_I etc
          
          hbcOut[i,"Psize"] = mean(psize[i1:i2])
         
          hbcOut[i,"negcases_i"] = sum(new_I[i1:i2,1:5]+new_NI[i1:i2,1:5])
          hbcOut[i,"poscases_i"] = sum(new_IH[i1:i2,1:5]+new_NIH[i1:i2,1:5])
          hbcOut[i,"negdeaths_i"] =sum(TBDeaths[i1:i2,1:5])
          hbcOut[i,"posdeaths_i"] = sum(TBDeathsH[i1:i2,1:5])
          
          hbcOut[i,"negcases_c"] = sum(new_I[i1:i2,1:14]+new_NI[i1:i2,1:14])
          hbcOut[i,"poscases_c"] = sum(new_IH[i1:i2,1:14]+new_NIH[i1:i2,1:14])
          hbcOut[i,"negdeaths_c"] =sum(TBDeaths[i1:i2,1:14])
          hbcOut[i,"posdeaths_c"] = sum(TBDeathsH[i1:i2,1:14])
          
          hbcOut[i,"negcases_a"] = sum(new_I[i1:i2,15:Mnage]+new_NI[i1:i2,15:Mnage])
          hbcOut[i,"poscases_a"] = sum(new_IH[i1:i2,15:Mnage]+new_NIH[i1:i2,15:Mnage])
          hbcOut[i,"negdeaths_a"] =sum(TBDeaths[i1:i2,15:Mnage])
          hbcOut[i,"posdeaths_a"] = sum(TBDeathsH[i1:i2,15:Mnage])
          
          hbcOut[i,"negcases_a_a"] = sum(new_I[i1:i2,6:Mnage]+new_NI[i1:i2,6:Mnage])
          hbcOut[i,"poscases_a_a"] = sum(new_IH[i1:i2,6:Mnage]+new_NIH[i1:i2,6:Mnage])
          hbcOut[i,"negdeaths_a_a"] =sum(TBDeaths[i1:i2,6:Mnage])
          hbcOut[i,"posdeaths_a_a"] = sum(TBDeathsH[i1:i2,6:Mnage])
    ...
    if (k < yearend){ 
      for (i in (2+(1/dt)*(k-year1)):((1/dt)*(k-year1)+1/dt)){
        ...
        if(i == ((1/dt)*(k-year1)+1/dt)){
          ...
          TBI[(k-year1+1),2]<-100000*sum(new_IH[i1:i2,],new_NIH[i1:i2,])/mean(psize[i1:i2])
          #print(c("IH",sum(new_IH[i,],new_NIH[i,]),TBI[i,2]))
          TBM[(k-year1+1),1]<-100000*sum(TBDeaths[i1:i2,])/mean(psize[i1:i2])
          TBM[(k-year1+1),2]<-100000*sum(TBDeathsH[i1:i2,])/mean(psize[i1:i2])
        }
...
## Fit function & Cost function

Fit4<-function(cccing){
  library(dfoptim)
  # output
  output<-matrix(0,17,1)
  FIT<-matrix(0,4,1)
  # For this country... 
  cntry<-countries[cccing]
  Ana<-matrix(0,1,6);
  Ana[1:2]<-Popsize[1:2,cntry];
  Ana[3]<-as.numeric(TBIm[cntry]);
  Ana[4]<-as.numeric(TBMm[cntry]);
  Ana[5]<-as.numeric(TBIHIVm[cntry]);
  Ana[6]<-as.numeric(TBMHIVm[cntry]);
  # Fit initial run
  if( file.exists('Initialfit.csv')){
    Ini<-as.matrix(read.csv('Initialfit.csv'));dimnames(Ini)<-NULL
    O<-Ini[,match(cntry,Ini[1,])]
    out<-as.numeric(O[2:5])
    prop<-as.numeric(O[6:9])                         
  } else {
    print('running initial fit')
    O<-FitHIVneg(cntry,c(0,0,0,0),0.01)
    out<-O$FIT;prop<-O$prop;
  }
  
  # Store output (Fit and prop)
  output[11:14]<-out # Top - want to be actual values
  output[7:10]<-prop # Prop needed for later
  assign('prop',prop,envir = .GlobalEnv)
  print(c("1st fit",out))
  
  # Limits for parameter values
  ll<-matrix(0,1,6);upp<-matrix(0,1,6);  
  ll[1]<-  -1; upp[1]<-out[1]+10000; # psize 1900
  ll[2]<-  -1; upp[2]<- 1            # ratio mortality in HIV negatives
  ll[3]<-   1; upp[3]<-out[3]+1000;  # neta - number of contacts
  ll[4]<-  -1; upp[4]<- 1            # ratio of TB mortality in HIV negatives
  ll[5]<-   0; upp[5]<- 1.5          # factor multiplication of CDR in HIV positives
  ll[6]<-  -1; upp[6]<- 1            # ratio of progression parameters in HIV positives
  print(c(ll,upp))
  
  assign('p0',out[1],envir = .GlobalEnv)
  outt<-c(out[2:4],0.8,0.15)
  print(c('outt',outt))
  # Attempt fiting 
  print(c('prop',prop))
  op1<-nmkb(outt, Cost4, lower=ll[-1],upper=upp[-1])
  print(op1)
  output<-c(output,op1$value,op1$convergence,op1$message)
  # Save outputs
  output[1:6]<-c(p0,op1$par)
  FIT<-output[1:6]
  
  # Differences
  X<-FitGo(cntry,c(1,0,0),c(p0,op1$par),c(1,0.5,0.01),c(1900,2050),0,C)
  XP1<-X$PSIZE[(2009-year1)*(1/dt)+1]-Popsize[1,cntry]
  XP2<-X$PSIZE[(2050-year1)*(1/dt)+1]-Popsize[2,cntry]
  TBId<-X$TBI[2009-year1+1]-as.numeric(TBIm[cntry])
  TBMd<-X$TBM[2009-year1+1]-as.numeric(TBMm[cntry])
  TBIHd<-X$TBIH[2009-year1+1]-as.numeric(TBIHIVm[cntry])
  TBMHd<-X$TBMH[2009-year1+1]-as.numeric(TBMHIVm[cntry])
  print(c('XP1',XP1))
  LS<-Cost4(c(op1$par))
  output[13:17]<-c(XP1,XP2,TBId,TBMd,LS)
  
  return(list(out=output,FIT=FIT))
  
}

Cost4 <- function(P){
  if (length(P)==2) {FIT<-c(); FIT[1]<-p0 ; FIT[2:3]<-P[1:2]; FIT[4]<-rmortTB }
  if (length(P)==4) {FIT<-P} 
  if (length(P)==3) {FIT<-c(); FIT[1]<-p0 ; FIT[2:4]<-P[1:3] }
  if (length(P)==5) {FIT<-c(); FIT[1]<-p0 ; FIT[2:6]<-P}
  ...
  X<-FitGo(cntry,c(1,0,0),FIT,c(1,0.5,0.01),c(1900,2050),0,C)
  psize2009<-X$PSIZE[(2009-year1)*(1/dt)+1]
  TBM2009<-X$TBM[2009-year1+1]
  
  if ((!is.na(psize2009)) && (!is.na(TBM2009))){ # If psize in 2009 is not NaN then fit p1900 and rmort
    assign('p0',p0*Ana[1]/psize2009,envir=.GlobalEnv)
    FIT[1]<-p0
    #print(c('p0',p0,'rmortTB',rmortTB,FIT))  
    p0old<-1000
    for (i in 1:10){
      print(c("while",i,psize2009-Ana[1],psize2009,Ana[1],p0,p0old))   
      if(is.na(psize2009-Ana[1]) || (abs(psize2009-Ana[1])<100)){break}
      
      X<-FitGo(cntry,c(1,0,0),FIT,c(1,0.5,0.01),c(1900,2050),0,C)
      psize2009<-X$PSIZE[(2009-year1)*(1/dt)+1]
      if(is.na(psize2009-Ana[1]) || psize2009 < 0){assign('p0',p0old,envir=.GlobalEnv); break}
      assign('p0old',p0,envir=.GlobalEnv)
      assign('p0',FIT[1]*Ana[1]/psize2009,envir=.GlobalEnv)
      FIT[1]<-p0
    }
  }
  
  print(c('p0f',p0,'rmortTBf',rmortTB))
  
  X<-FitGo(cntry,c(1,0,0),FIT,c(1,0.5,0.01),c(1900,2050),0,C)
  psize2009<-X$PSIZE[(2009-year1)*(1/dt)+1]
  psize2050<-X$PSIZE[(2050-year1)*(1/dt)+1]
  tbI<-X$TBI[2009-year1+1];tbM<-X$TBM[2009-year1+1]
  tbIH<-X$TBIH[2009-year1+1];tbMH<-X$TBMH[2009-year1+1]
  
  print(c(tbI,tbIH,psize2009, psize2050))
  
  if (length(P)==2){LS <- ((psize2050-Ana[2])/Ana[2])^2+((tbI-Ana[3])/Ana[3])^2}
  if (length(P)==3){LS <- ((psize2050-Ana[2])/Ana[2])^2+((tbI-Ana[3])/Ana[3])^2+((tbM-Ana[4])/Ana[4])^2}
  if (length(P)==4){LS <- ((psize2009-Ana[1])/Ana[1])^2+((psize2050-Ana[2])/Ana[2])^2+((tbI-Ana[3])/Ana[3])^2+((tbM-Ana[4])/Ana[4])^2}
  if (length(P)==5){LS <- ((psize2050-Ana[2])/Ana[2])^2+((tbI-Ana[3])/Ana[3])^2+((tbM-Ana[4])/Ana[4])^2+((tbIH-Ana[5])/Ana[5])^2+((tbMH-Ana[6])/Ana[6])^2}
...
DALY<-function(cntry,pvxd,pc,vw,pdots,pmdr,mdrprop,mdrproptr,X){
  # Constant parameters
  dr=0.03;
  L=0.4109589;
  d_hivneg=0.331;
  ...
  YLDneg.x <- YLDnegpp * X[,"HIVnegcases.x"]
  YLDneg.y <- YLDnegpp * X[,"HIVnegcases.y"]
  YLDpos.x <- YLDpospp * X[,"HIVposcases.x"]
  YLDpos.y <- YLDpospp * X[,"HIVposcases.y"]
  
  discbb<-(K*C*exp(dr*a)/((dr+B)^2)*(exp(-(dr+B)*(L+a))*{-(dr+B)*(L+a)-1}-exp(-(dr+B)*a)*{-(dr+B)*a-1}) + ((1-K)/dr)*(1-exp(-dr*L)))
  discYLDnegpp <- d_hivneg*discbb
  discYLDpospp <- d_hivpos*discbb
  
  discYLDneg.x <- discYLDnegpp * discTBhivneg.x
  discYLDneg.y <- discYLDnegpp * discTBhivneg.y
  discYLDpos.x <- discYLDpospp * discTBhivpos.x
  discYLDpos.y <- discYLDpospp * discTBhivpos.y
  
  TotalYLD.x <- YLDneg.x + YLDpos.x
  TotalYLD.y <- YLDneg.y + YLDpos.y
  
  discTotalYLD.x <- discYLDneg.x + discYLDpos.x
  discTotalYLD.y <- discYLDneg.y + discYLDpos.y
  
  # **********************************  DALYS
  TotalDALYS.x <- TotalYLD.x + TotalYLL.x 
  TotalDALYS.y <- TotalYLD.y + TotalYLL.y 
  ...
  discDotscost.x<-Dotscost.x/(1+dr)^(X[,"Year"]-2024)
  discMDRcost.x<-MDRcost.x/(1+dr)^(X[,"Year"]-2024)
  discDotscost.y<-Dotscost.y/(1+dr)^(X[,"Year"]-2024)
  discMDRcost.y<-MDRcost.y/(1+dr)^(X[,"Year"]-2024)
